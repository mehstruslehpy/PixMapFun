...
void fillcolor_triangle(Pixel color,Triangle tri,Pixel** frame_buffer,int x_bound,int y_bound)
{
	double xmin=floor(fmin(tri.vert1.x,fmin(tri.vert2.x,tri.vert3.x)));
	double xmax=ceil(fmax(tri.vert1.x,fmax(tri.vert2.x,tri.vert3.x)));
	double ymin=floor(fmin(tri.vert1.y,fmin(tri.vert2.y,tri.vert3.y)));
	double ymax=ceil(fmax(tri.vert1.y,fmax(tri.vert2.y,tri.vert3.y)));
	for (int y=ymin;y<=ymax;y++)
		for (int x=xmin;x<=xmax;x++)
		{
			double alpha = fij(tri.vert2,tri.vert3,x,y)/fij(tri.vert2,tri.vert3,tri.vert1.x,tri.vert1.y);
			double beta = fij(tri.vert3,tri.vert1,x,y)/fij(tri.vert3,tri.vert1,tri.vert2.x,tri.vert2.y);
			double gamma = fij(tri.vert1,tri.vert2,x,y)/fij(tri.vert1,tri.vert2,tri.vert3.x,tri.vert3.y);
			//this can be modified for blending colors starting from vertices
			//but we only need to fill a single color for our intended goal
			if (alpha>0&&beta>0&&gamma>0) 
			{
				//Pixel new_color = (Pixel){alpha,beta,gamma};
				double red = alpha*color.red+beta*color.red+gamma*color.red;
				double grn = alpha*color.grn+beta*color.grn+gamma*color.grn;
				double blu = alpha*color.blu+beta*color.blu+gamma*color.blu;
				Pixel new_color=(Pixel){red,grn,blu};
				plot_fbuf(x,y,new_color,frame_buffer,x_bound,y_bound);
			}
		}
}
void fillcolor_convex_polygon(Pixel color,Polygon poly,Pixel** frame_buffer,int x_bound,int y_bound)
{
	...
	tri.vert2 = poly.pt[1];
	tri.vert3 = poly.pt[2];
	fillcolor_triangle(color,tri,frame_buffer,x_bound,y_bound);
	for (int i = 2;i<poly.count-1;i++)
	{
		tri.vert2 = poly.pt[i];
		tri.vert3 = poly.pt[i+1];
		fillcolor_triangle(color,tri,frame_buffer,x_bound,y_bound);
...
void draw_wireframe(Pixel color,Wireframe w,Pixel** frame_buffer, int x_bound ,int y_bound)
{
	for (int i=0;i<w.count;i++)
	{
		draw_polygon(color,w.poly[i],frame_buffer,x_bound,y_bound);
	}
}
