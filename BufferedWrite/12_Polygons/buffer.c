...
{
	debug_print_polygon("DEBUG: draw_polygon():",&p);
	for (int i=0;i<p.count-1;i++)
	{
		printf("DEBUG: draw from pt %d to pt %d\n",i,i+1);
		draw_segment(color,p.pt[i],p.pt[i+1],frame_buffer,x_bound,y_bound);
	}
	printf("DEBUG: draw from pt %d to pt 0\n",p.count-1);
...
{
	debug_print_vector("DEBUG: draw from:",&pt1);
	debug_print_vector("DEBUG: draw to:",&pt2);
