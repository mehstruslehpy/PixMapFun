...
Pixel julia_set(double xin,double yin,double* params,int pcnt)
{
...
int main(int argc, char** argv)
{
	...
	if (argc!=6)
	{
		printf("Usage: ./julia <zoom> <x-shift> <y-shift> <cx> <cy>\n");
		printf("\texample ./julia 1 0 0 -0.7 0.27015\n");
		return EXIT_FAILURE;
	}
	...
	double params1[8] = { atof(argv[1]), //zoom
		X_BOUND,
		Y_BOUND,
		atof(argv[2]), //movex
		atof(argv[3]), //movey
	...
	process_fbuf(frame_buffer,julia_set,params1,sizeof(params1),X_BOUND,Y_BOUND);
