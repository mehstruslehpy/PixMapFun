...
Pixel multibrot_set(double xin,double yin,double* params,int pcnt)
{
	...
	complex c = scale_to_interval(xin,params[0],params[1],-2.5,1.0)
		+ I*scale_to_interval(yin,params[2],params[3],-1.0,1.0);
	int d = params[4];
	complex z = c;
	...
	/*
	   ...
	   */
	int iteration = 0;
	int max_iteration = params[5];

	while (cabs(z)<=2*2 && iteration<max_iteration)
	{
		/*
...
int main(int argc, char** argv)
{
	...
	if (argc!=7)
	{
		printf("Usage: ./multibrot <int> <int> <int> <int> <double> <int>\n");
		printf("\texample: ./multibrot 0 1024 0 1024 2 255\n");
		return EXIT_FAILURE;
	}
	//MANDELBROT SET
	...
	double params2[6] = {atof(argv[1]),atof(argv[2]),atof(argv[3]),atof(argv[4]),atof(argv[5]),atof(argv[6])};
	printf("DEBUG: process frame buffer.\n");
	process_fbuf(frame_buffer,multibrot_set,params2,sizeof(params2),X_BOUND,Y_BOUND);
...
void write_fbuf_to_disk(Pixel frame_buffer[X_BOUND][Y_BOUND],int x_bound,int y_bound)
{
	...
	strncat(framename,"multibrot.ppm",13);
