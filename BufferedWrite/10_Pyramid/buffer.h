...
//allocate and return a triangle
void draw_triangle(Pixel,Triangle,Pixel**,int,int);
void draw_segment(Pixel,Tuple,Tuple,Pixel**,int,int);
//some vector math functions
Tuple cross_product(Tuple,Tuple);
double dot_product(Tuple,Tuple);
Tuple normalize(Tuple);
Tuple scalar_mul(double,Tuple);
//some projection functions
Triangle project_triangle(Triangle,Tuple,Tuple,Tuple,ViewVolume,int,int);
Tuple project_tuple(Tuple,Tuple,Tuple,Tuple,ViewVolume,int,int);
#endif
